...
function toMysqlDate($date, $format = 'm-d-Y', $returnFormat = 'Y-m-d'){
    $newDate = DateTime::createFromFormat($format, $date);
    // dd(DateTime::getLastErrors());
    return $newDate ? $newDate->format($returnFormat) : $date;
}
function toMysqlDateTime($date, $format = 'm-d-Y', $returnFormat = 'Y-m-d', $timezone_offset = 0){
    // dd($timezone_offset);
...
function toAppDate($date, $format = 'm-d-Y'){

...
class UploadService {
    ...
    private $supportedTypes = ['csv', 'xml', 'png', 'jpg'];
    ...
    public function upload($newFileName = '', \Closure $closure = null){
        if(!is_null($closure)){
            ...
            if($newFileName != ''){
                ...
                $this->uploadedName = '_'.time().Str::random(15).'.'.$this->handler->getClientOriginalExtension();
            }
            $this->fileSize = $this->handler->getSize();
            ...
            // return $this->handler->storePubliclyAs($this->getUploadPath(), $this->getUploadedName(), config('filesystems.default'));
